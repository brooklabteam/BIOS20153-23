...
sim.model.TS <- function(par, model){
  
  #write down the par as inputs into each model
  if(model=="MSIRN"){
    model.sim <- sim.met.MSIRN.age.TS(burnin=par$burnin,
                                   sim_pop=par$sim_pop,
                                   yrs=par$yrs,
                                   ntyr=par$ntyr,
                                   s=par$s,
    ...
    model.sim  = sim.met.MSIR.age.TS(burnin=par$burnin,
                                  sim_pop=par$sim_pop,
                                  yrs=par$yrs,
                                  ntyr=par$ntyr,
                                  s=par$s,
...
sim.met.MSIR.age.TS <- function(burnin, sim_pop, yrs, ntyr, age.brk, s, beta, sero, recov, mort, mort_juv, adult_fec, wane, slope.wane, sigma, mu.sick, add.inf.mort, model){
  #first pull out your data...
  c=4
  # dat.tmp <- subset(dat.age1, species==species1 & type==type1)
  #then run model for 100 years 
...
sim.met.MSIRN.age.TS <- function(burnin, sim_pop, yrs, ntyr,  s, beta, age.brk, recov, mort, mort_juv, adult_fec, wane, boost, slope.wane, sigma, mu.sick, add.inf.mort, model, N_stat){
  #first pull out your data...
  c=5
  # dat.tmp <- subset(dat.age1, species==species1 & type==type1)
  #then run model for 100 years 
...
log.lik.wane.age <- function(par, data, model, par.dat){
  ...
  if(is.na(unique(out$seropos)[1])){
    ...
    print(-ll)
    #if Inf, change to a very lage number
    if(-ll==Inf){
      ll <- -10000000
    }
    print(-ll)
